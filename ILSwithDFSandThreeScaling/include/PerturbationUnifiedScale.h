void PerturbationUnifiedScale(FullSol* FullSolution, CommonList* CurCommonLists, short int NoOfPoliciesToChangeInPerturbation, float LinksDelay, ofstream &myfile1, ofstream &myfile2,  ChangedPolicies* TrackChangedPolicies){
	...
	for (int k = 0; k< NoOfPoliciesToChangeInPerturbation; k++){
		...
		if(FullSolution->ScaleAccepted.size() > 0){
			...
			success = DFSUnifiedScaling(FullSolution, CurCommonLists, PolicyID, startNF, CurrentBW, LinksDelay, &SelectedPolicyTemp, &SelectedPolicyOriginal,  TrackChangedPolicies);
