void LSIncrementalScale(FullSol* InitialFullSolution, CommonList* CurCommonLists, float LinksDelay, int ServerCap, int InPathLinks, ofstream &myfile2, ChangedPolicies* TrackChangedPolicies ){
	...
	for (auto local_it = InitialFullSolution->ScaleRejected.begin(); local_it!= InitialFullSolution->ScaleRejected.end();){
		...
		int ParentID = CurCommonLists->PolicyList[PolicyID].Parent;
		int ParentOriginalBW = CurCommonLists->PolicyList[ParentID].CurrentBW;
		...
		while( (AffordableN == false) && ( NoPointOfScaling == false)){
			...
			if(AffordableN == false){
				...
				if( (nextTotBW <= ParentOriginalBW) || (nextTotBW > BWToCheckTemp)){
		...
		if(AffordableN == true){
			...
			while ((success == false) && (NoPointOfScaling == false)){
				...
				// even though this is Incremental, since this is inside repeated procedure, child policy is already created before and v can use unified
				if(OBW != NewBWFull){
					success = DFSUnifiedScaling(InitialFullSolution, CurCommonLists, PolicyID, startNF, NewBWFull, LinksDelay, &SelectedPolicyTemp, &SelectedPolicyOriginal, TrackChangedPolicies);
				}
				OBW = NewBWFull;
