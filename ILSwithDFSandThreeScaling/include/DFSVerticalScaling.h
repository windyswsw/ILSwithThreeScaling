bool DFSVerticalScaling(FullSol* InitialFullSolution, CommonList* CurCommonList, int policyIndex, int CurrentNF, int BW, float LinksDelay, PartialSol* newVMMap, PartialSol* SelectedPolicyOriginal, ChangedPolicies* TrackChangedPolicies ){
	...
	// Check whether we can put it in the original server
			...
			if(CurrentNF == 0 ){ // first NF in the chain
				...
				if( NoInPath == false){ // there is an InPath
					...
					if(DelayOK == true){ // obj is better, so proceed
						...
						Assigned = true;						
						sucess = DFSVerticalScaling(InitialFullSolution, CurCommonList, policyIndex, next_pos, TrafficToNextNF, LinksDelay, newVMMap, SelectedPolicyOriginal, TrackChangedPolicies); // call to next NF
					}
				}
			}
