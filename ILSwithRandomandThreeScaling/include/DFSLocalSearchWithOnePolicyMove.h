bool DFSLocalSearchWithOnePolicyMove(FullSol* InitialFullSolution, CommonList* CurCommonList, int policyIndex, int CurrentNF, int BW, float LinksDelay, PartialSol* newVMMap, PartialSol* SelectedPolicyOriginal ){
			...
			if(CurrentNF == 0 ){ // first NF in the chain
				...
				if( NoInPath == false){ // there is an InPath
					...
					if(DelayOK == true){ // obj is better, so proceed
						...
						Assigned = true;						
						sucess = DFSLocalSearchWithOnePolicyMove(InitialFullSolution, CurCommonList, policyIndex, next_pos, TrafficToNextNF, LinksDelay, newVMMap, SelectedPolicyOriginal); // call to next NF
					}
				}
			}
