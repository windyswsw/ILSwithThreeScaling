bool DFSIncrementalScaling(FullSol* InitialFullSolution, CommonList* CurCommonList, int policyIndex, int CurrentNF, int BW, float LinksDelay, PartialSol* newVMMap, PartialSol* SelectedPolicyOriginal, ChangedPolicies* TrackChangedPolicies ){
			...
			if(CurrentNF == 0 ){ // first NF in the chain
				...
				if( NoInPath == false){ // there is an InPath
					...
					if(DelayOK == true){ // obj is better, so proceed
						...
						Assigned = true;						
						sucess = DFSIncrementalScaling(InitialFullSolution, CurCommonList, policyIndex, next_pos, TrafficToNextNF, LinksDelay, newVMMap, SelectedPolicyOriginal, TrackChangedPolicies); // call to next NF
					}
				}
			}
					...
					for (int h=(NoOfPaths-1); h>= 0; h--){
						...
						if(NoNFPath == false){ // there is a path so continue
							...
							if(DelayOK == true){ // obj is better, so proceed
								...
								InitialFullSolution->VectorOfPartialSols.push_back(TempP);
