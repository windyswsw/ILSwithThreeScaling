bool DFSUnifiedScaling(FullSol* InitialFullSolution, CommonList* CurCommonList, int policyIndex, int CurrentNF, int BW, float LinksDelay, PartialSol* newVMMap, PartialSol* SelectedPolicyOriginal, ChangedPolicies* TrackChangedPolicies ){
	...
	if(SelectedPolicyOriginal->VMsForAPolicy.size()>0){ // if original VMMap not empty
		NFAllocation NFN = SelectedPolicyOriginal->VMsForAPolicy[CurrentNF];
		short int NFSvrN = NFN.ServerID;
		bool NoServer = ServerCapCheck(&InitialFullSolution->CurLists,NFSvrN, NFCapacity); // check whether we can put in the original server
		if(NoServer == false){
			ServerArr.push_back(NFSvrN);
		}
	}
	...
	if( ServerArr.size() > 1){
		...
		if( CurrentNF != 0){
			...
			if(( ServerArr.size() > 2) && (RandomStart != TotSvr)){
				swap(ServerArr[1], ServerArr[RandomStart+1]);
			}
			else if (( ServerArr.size() > 2) && (RandomStart == TotSvr)){
				swap(ServerArr[1], ServerArr[0]);
			}
		}
	}
			...
			if(CurrentNF == 0 ){ // first NF in the chain
				...
				if( NoInPath == false){ // there is an InPath
					...
					if(DelayOK == true){ // obj is better, so proceed
						...
						Assigned = true;						
						sucess = DFSUnifiedScaling(InitialFullSolution, CurCommonList, policyIndex, next_pos, TrafficToNextNF, LinksDelay, newVMMap, SelectedPolicyOriginal, TrackChangedPolicies); // call to next NF
					}
				}
			}
					...
					for (int h=(NoOfPaths-1); h>= 0; h--){
						...
						if(NoNFPath == false){ // there is a path so continue
							...
							if(DelayOK == true){ // obj is better, so proceed
								...
								InitialFullSolution->VectorOfPartialSols[policyIndex].AllocatedBW = newVMMap->AllocatedBW;
								...
								TempP.AllocatedBW = newVMMap->AllocatedBW;
								...
								OriginP.AllocatedBW = SelectedPolicyOriginal->AllocatedBW;
